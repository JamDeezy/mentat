...
// Line
//
function Line(selector, data, key, scale, color, tooltip) {
  // Scope our variables
  var line       = this;
  ...
  if (line.color instanceof Function) {
    ...
    line.z.range(line.scale)
      .domain(line.key.metric);

  } else {
    // Return our own default colors
    line.z.range(DEFCOLORS)
      .domain(line.key.metric);
  }
  ...
  var zero = d3.svg.line()
    .x(function(d) { return line.x(d.key) })
    .y(function(d) { return line.y(yRange[0] * 0.91) });
